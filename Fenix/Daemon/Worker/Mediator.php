 ...
 * an instance of WorkerInterface to the DaemonBase::worker() method) the Daemon creates a Mediator instance and
 ...
 * @author Shane Harter
 */
abstract class Mediator implements TaskInterface
{
    /**
    ...
    */
    ...
    public function __construct($alias, DaemonBase $daemon, WorkerViaInterface $via) {
        $this->alias            = $alias;
        $this->daemon           = $daemon;
        $this->via              = $via;
    ...
    public function debug() {
        ...
        $this->via->prompt_prefix_callback = function($method, $args) use($alias) {
            return sprintf('%s %s %s', $alias, getmypid(), (DaemonBase::is('parent')) ? 'D' : 'W');
        };
    ...
    public function breakpoint($prompt = '', $indent = 0) {
        if (!DaemonBase::get('debug_workers'))
            return true;
    ...
    public function setup() {
        ...
        if (DaemonBase::is('parent')) {
            ...
            $this->daemon->on(DaemonBase::ON_PREEXECUTE,   array($this, 'run'));
            $this->daemon->on(DaemonBase::ON_IDLE,         array($this, 'garbage_collector'), ceil(120 / ($this->workers * 0.5)));  // Throttle the garbage collector
            $this->daemon->on(DaemonBase::ON_SIGNAL,       array($this, 'dump'), null, function($args) {
            ...
            $this->daemon->on(DaemonBase::ON_SIGNAL, $event_restart, null, function($args) {
                return $args[0] == SIGUSR1;
            });
    ...
    /**
     * Satisfy the TaskInterface interface.
     */
     ...
     * Note: Called only in the parent (daemon) process, attached to the DaemonBase::ON_PREEXECUTE event.
     *
     * @return void
     */
    public function run() {
    ...
    public function start() {
        ...
        while (!DaemonBase::is('parent') && !DaemonBase::is('shutdown') && !$recycle) {
    ...
    public function garbage_collector() {
        ...
        foreach ($this->calls as $call_id => &$call) {
            if ($call->gc() && DaemonBase::is('parent'))
        ...
        if (!DaemonBase::is('parent') || count($called) == 0)
            return;
    ...
    public function count_error($type) {
        ...
        if ($this->error_counts[$type] > $this->error_thresholds[$type][(int)DaemonBase::is('parent')])
    ...
    /**
     * Helper function to retrieve the selected process from the DaemonBase process registry
     * @param $pid
    ...
    /**
     * Helper function to retrieve all of this workers processes from the DaemonBase process registry
     * @return Core_Lib_Process[]
     */
